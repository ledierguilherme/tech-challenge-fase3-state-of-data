# Deploy do job bronze: sobe o script pro bucket de assets do glue, cria/atualiza o job,
# roda e fica monitorando até terminar. Se falhar tenta puxar o traceback do cloudwatch.
...
SCRIPT_FILE_NAME = "glue-state-of-data-bronze.py"
...
if not Path(script_path).exists():
    ...
    print(f"   💡 Certifique-se de que o arquivo '{SCRIPT_FILE_NAME}' existe na pasta '02_bronze'")
    sys.exit(1)

print(f"   ✅ Script encontrado\n")

# na primeira vez o bucket aws-glue-assets-* nem existia na conta do lab, por isso o create
...
try:
    ...
    # create aceita Tags, update não (e não pode ter Name dentro do JobUpdate)
    create_job_config = {
        "Name": JOB_NAME,
        "Role": role_arn,
        "Command": {
        ...
        "Timeout": 30,
        "MaxRetries": 0,
        "Description": "Bronze: lê os CSVs State of Data BR (2023, 2024, 2025) e grava Parquet particionado por ano_pesquisa com catalogação no Glue",
        "Tags": {
...
def print_cloudwatch_errors(job_run_id: str, max_lines: int = 80):
    # o ErrorMessage do get_job_run vem truncado, então vou buscar o traceback nos log groups do glue
    candidates = [
...
try:
    ...
    while elapsed_time < max_wait_time:
        try:
            ...
            # só imprime quando o estado muda pra não poluir
